...
prerrequisitos: [Chapter 1, credenciales y perfiles de CLI/boto3, IAM (roles de servicio y políticas), anatomía de un training job de SageMaker, S3 (buckets, claves y prefijos), SQL básico, Python y pandas]
no-se-usa-aqui: [feature engineering (Chapter 3), algoritmos en detalle (Chapter 4), hiperparámetros y evaluación (Chapter 5), endpoints e inferencia (Chapter 6), KMS y diseño de red más allá de pasar identificadores (Chapter 8; red como caja negra declarada en la sección de MSK)]
region: us-east-1
versiones:
  python: "3.12.6"
  boto3: "1.43.99"
...
tags: [aws, mla-c01, ingesta, almacenamiento, firehose, kinesis, msk, flink, datasync, glue, s3, athena, efs, fsx, ebs, rds, dynamodb, boto3]
---
...
*FIGURE 2.2 The data engineering lifecycle.*
...
*FIGURE 2.3 AWS storage systems.*
...
`df.to_json(..., orient="records", lines=True)` escribe *JSON Lines*: un objeto
...
`metadata.row_group(0).column(2)`: un archivo Parquet se divide en *row groups*
(bloques de filas) y, dentro de cada uno, en *column chunks* (el tramo de una
...
El esquema es un documento JSON: un `record` con nombre, *namespace* (espacio de
...
en orden *little-endian* (`<`): el número mágico y la longitud. En la salida
...
cuando cada carga es un mensaje *protobuf* `Record` con el vector de
...
canal de un *training job*. SageMaker no convierte nada: copia los objetos tal
...
As you may have noticed, Amazon S3 is the most flexible service because it can be used for all three different data categories. Additionally, you can supplement Amazon S3 with Amazon Athena and query your data directly from S3 without formatting the data or managing the infrastructure.
...
Un stream de Kinesis se divide en *shards*: particiones con capacidad fija,
...
produce `ResourceNotFoundException`; el *waiter* consulta el estado hasta que es
`ACTIVE`.
...
Un consumidor lee cada shard con un *iterador*: un marcador de posición que la
...
se recupera tras caerse. El nombre es histórico: el *trim horizon* es el borde
...
`create_event_source_mapping` crea el *event source mapping*: un lector que
...
por `UPDATING` mientras tanto; por eso el *waiter* entre llamadas: una segunda
...
El bucle de `describe_cluster_v2`: el cliente `kafka` no tiene *waiters*, y el
...
mecanismo `OAUTHBEARER` le pide a un objeto un *token* cada vez que se conecta,
...
particiones y Kafka recuerda por qué *offset* (posición) va cada grupo. Un
...
visto eventos de las 12:01:05. Sin *watermark*, una ventana definida sobre
...
«estado» del que habla el texto, y Flink los guarda en *checkpoints* para no
...
*checkpoints* periódicos con los valores del servicio. De ellos depende la
...
`"ParallelismConfiguration"`: el servicio cobra por KPU (*Kinesis Processing
Unit*, otra vez el nombre antiguo); `Parallelism` 4 con `ParallelismPerKPU` 1
...
Una transferencia en DataSync son tres recursos: una *location* de origen, una de
destino y una *task* que las une; cada ejecución de la tarea es una *task
execution*.
...
el script no menciona ni rutas ni formato de entrada. Un *DynamicFrame* es la
...
`transformation_ctx` y `job.commit()` son los *job bookmarks*: Glue anota qué
...
`WorkerType="G.1X"`, `NumberOfWorkers=10`: cada *worker* G.1X es un ejecutor de
...
esta etapa. Es exactamente la combinación que pregunta la *review question* 7.
...
*FIGURE 2.4 Amazon S3 use in the ML lifecycle.*
...
archivos grandes en trozos que sube en paralelo (*multipart upload*), y es la
...
La *review question* 2 pide datos procesados accesibles al instante durante
...
necesita su propio tipo de bucket, el *directory bucket*, fijado a una zona de
disponibilidad.
...
*ID* de zona, no un nombre como `us-east-1a`: los nombres se asignan distinto en
...
ubicación de resultados (ni en la llamada ni en el *workgroup*), la consulta
...
`CREATE TABLE ... WITH (format = 'PARQUET', ...) AS SELECT` (*CTAS*) escribe el
...
*FIGURE 2.5 Amazon EFS use in the ML lifecycle.*
...
sistema de archivos es accesible a través de un *mount target*, una interfaz de
red en cada subred.
...
`put_lifecycle_configuration`: el *lifecycle management* del texto. Los
...
Internet, así que para escribir el modelo en S3 la VPC necesita un *endpoint*
...
*FIGURE 2.6 Amazon FSx for Lustre use in the ML lifecycle.*
...
archivo dispara la copia desde S3 (el *lazy loading* del texto); los siguientes
...
`"DeploymentType": "SCRATCH_2"` es el *scratch file system* del texto: más
...
La carga completa (*one-time load*) se hace desde un cliente que tenga montado
...
sobrevive a fallos de servidor: el *persistent file system* del texto.
...
*FIGURE 2.7 Training data load times comparison.*
...
rendimiento), una o más *storage virtual machines* (SVM, cada una con sus
...
| KPU | Unidad de cómputo y facturación de una aplicación Flink | *Kinesis Processing Unit*, del mismo nombre anterior |
| `transformation_ctx` | Etiqueta que Glue usa para recordar qué datos ya procesó un paso del job | Contexto de transformación de los *job bookmarks* |
...
| `hsm_restore` | Orden de Lustre que trae el contenido de un archivo desde el repositorio enlazado (S3) | *Hierarchical Storage Management* |
| `HASH` / `RANGE` | Clave de partición y clave de ordenación de DynamoDB | Nombres antiguos; la documentación actual usa *partition key* y *sort key* |
